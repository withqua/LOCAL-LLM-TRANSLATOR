...
GPT-5.5 계열 모델을 사용할 때는 오래된 단계 나열식 프롬프트보다 결과 중심의 짧은 지시를 선호하세요. 좋은 프롬프트는 목표, 성공 기준, 중요한 제약, 사용 가능한 근거, 최종 응답 형식을 먼저 정의하고, 해결 경로는 모델이 효율적으로 선택할 수 있게 둡니다.

- 목표를 먼저 쓰세요: 사용자가 최종적으로 얻어야 하는 결과와 완료 조건을 명확히 합니다.
- 절대 규칙은 진짜 불변 조건에만 사용하세요: 보안, 개인정보, 출력 필드, 금지된 부작용처럼 어기면 안 되는 항목에 한정합니다.
- 판단이 필요한 영역은 결정 규칙으로 표현하세요: 언제 검색할지, 언제 질문할지, 언제 멈출지 같은 기준을 적습니다.
- 이전 모델용 장황한 프롬프트 스택을 그대로 복사하지 마세요: 과한 절차 지시는 탐색 공간을 좁히고 응답을 기계적으로 만들 수 있습니다.
- 추론 노력은 다시 평가하세요: GPT-5.5에서는 `low` 또는 `medium`으로 충분한 작업이 많으므로, 복잡도와 위험이 명확할 때만 더 높은 노력을 사용합니다.

도구를 쓰거나 여러 단계가 필요한 작업은 첫 도구 호출 전에 짧은 사용자 가시 업데이트를 남기세요. 요청을 이해했음을 밝히고 첫 단계를 1-2문장으로 설명하면 됩니다. 작업 중에는 긴 침묵을 피하고, 새로 알게 된 사실이나 다음 행동을 짧게 공유하세요.

프롬프트에 성격과 협업 방식을 넣을 때는 짧고 실용적으로 유지하세요:

- 성격은 말투, 온도, 직접성, 유머, 공감 수준을 정합니다.
- 협업 방식은 질문 시점, 가정 사용, 주도성, 검증 방식, 불확실성 처리 방식을 정합니다.
- 두 항목 모두 목표, 성공 기준, 도구 규칙, 중단 조건을 대신하지 않습니다.

근거가 필요한 답변이나 문서 기반 작업에는 검색 예산을 명시하세요:

- 일반 질의는 짧고 구별력 있는 키워드로 한 번 넓게 검색한 뒤, 핵심 답변에 충분한 근거가 있으면 멈춥니다.
- 필수 사실, 날짜, ID, 출처, 특정 문서, 비교 항목이 빠졌을 때만 추가 검색합니다.
- 표현을 다듬거나 부수적인 예시를 늘리기 위해 검색을 반복하지 마세요.
- 근거가 없다는 사실만으로 단정적인 부정을 만들지 말고, 확인된 범위와 남은 불확실성을 구분하세요.

창의적 초안, 슬라이드, 사용자-facing 문구, 요약문을 만들 때는 출처 기반 사실과 창의적 문장을 구분하세요. 제품 기능, 고객명, 수치, 일정, 로드맵, 경쟁 비교처럼 검증 가능한 주장은 제공되거나 검색된 근거에 기반해야 합니다. 근거가 부족하면 구체적 주장을 지어내지 말고, 일반화된 문장이나 명시적 플레이스홀더를 사용하세요.

긴 작업이나 구현 계획은 추적 가능해야 합니다. 필요한 경우 다음을 포함하세요:

- 요구사항과 각 요구사항을 다루는 위치.
- 관련 파일, API, 런타임 영역, 데이터 흐름.
- 검증 명령 또는 수동 점검 방법.
- 실패 동작, 개인정보 및 보안 영향.
- 구현 전에 답해야 하는 중요한 미해결 질문.

최종 응답은 간결하게 작성하세요. 일반 설명은 짧은 문단을 기본으로 하고, 목록은 비교, 단계, 검증 결과처럼 스캔성이 실제로 좋아질 때만 사용합니다. 사용자가 특정 형식이나 길이를 요청하면 그 선호를 우선하세요.
